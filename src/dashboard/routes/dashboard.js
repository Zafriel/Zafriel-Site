...
router.get("/guilds/:guildId", async (req, res) => {
  ...
  if (!guild)
    return res.redirect(
        `https://discordapp.com/oauth2/authorize?client_id=${client.user.id}&scope=bot&permissions=8&guild_id=${req.params.guildId}`
        );
...
router.post("/guilds/:guildId", async (req, res) => {
  const ak = req.body;
  GuildConfig.findOneAndUpdate({ guildId: req.params.guildId }, { $set: ak })
    .then(async () => {
      res.send(ak);
    })
    .catch((err) => {
